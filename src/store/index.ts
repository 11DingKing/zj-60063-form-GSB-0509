...
import type { FormConfig, Field, SubmissionRecord, FormTemplate, GroupField } from '@/types'
    ...
    selectedField: (state): Field | undefined => {
      ...
      const findField = (fields: Field[]): Field | undefined => {
        for (const field of fields) {
          ...
          if (field.type === 'group') {
            const found = findField((field as GroupField).children || [])
  ...
  actions: {
      ...
      const updateInFields = (fields: Field[]): boolean => {
        for (let i = 0; i < fields.length; i++) {
          ...
          if (fields[i].type === 'group') {
            if (updateInFields((fields[i] as GroupField).children)) {
              return true
            }
          }
        }
    ...
    deleteField(fieldId: string) {
      ...
      const deleteFromFields = (fields: Field[]): boolean => {
        ...
        for (const field of fields) {
          if (field.type === 'group' && deleteFromFields((field as GroupField).children)) {
            return true
          }
        }
        return false
      ...
      if (parentGroupId) {
        const moveInGroup = (fields: Field[]): boolean => {
          for (const field of fields) {
            ...
            if (field.type === 'group' && moveInGroup((field as GroupField).children)) {
              return true
            }
          }
          return false
      ...
      const reassignIds = (fields: Field[]): Field[] => {
        return fields.map((field) => {
          ...
          if (newField.type === 'group') {
            ;(newField as GroupField).children = reassignIds((newField as GroupField).children)
          }
          return newField
        })
      }
    ...
    loadDraft(formId: string) {
      return loadFromStorage<unknown>(STORAGE_KEY_DRAFT + formId, null)
    },
